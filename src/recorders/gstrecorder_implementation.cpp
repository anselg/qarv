...
class GstRecorder: public Recorder {
	...
	public:
		GstRecorder(QString outputFormat, QArvDecoder* decoder, 
		            QString fileName_, QSize size, int FPS, bool writeInfo) {
			numberOfFrames = 0;
			fileName = fileName_;
			if (!gstOK) return;
			QString informat;
			...
			QString cmdline("gst-launch-1.0 -e "
			                ...
			                "%5 ! "
			                "filesink location=%6");
			cmdline = cmdline.arg(informat, QString::number(FPS), 
...
Recorder* QArv::makeGstRecorder(QStringList plugins, 
	...
	if (OK)
		return new GstRecorder(pipelineFragment, decoder, 
		                       fileName, size, FPS, writeInfo);
	else
		return NULL;
}
