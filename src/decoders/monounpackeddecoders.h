...
class Mono8Format : public QObject, public QArvPixelFormat {
	...
	public:
		...
		QArvDecoder* makeDecoder(QSize size) { 
			return new MonoUnpackedDecoder<uint8_t, 8, ARV_PIXEL_FORMAT_MONO_8>(size); 
		}
};
...
class Mono10Format : public QObject, public QArvPixelFormat {
	...
	public:
		...
		QArvDecoder* makeDecoder(QSize size) { 
			return new MonoUnpackedDecoder<uint16_t, 10, ARV_PIXEL_FORMAT_MONO_10>(size); 
		}
};
...
class Mono12Format : public QObject, public QArvPixelFormat {
	...
	public:
		...
		QArvDecoder* makeDecoder(QSize size) { 
			return new MonoUnpackedDecoder<uint16_t, 12, ARV_PIXEL_FORMAT_MONO_12>(size); 
		}
};
