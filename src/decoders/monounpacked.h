...
class MonoUnpackedDecoder : public QArvDecoder
{
  ...
  static_assert(sizeof(InputType) <= sizeof(uint16_t), "InputType too large.");
...
private:
  ...
  typedef
    typename std::conditional<OutputIsChar, uint8_t, uint16_t>::type OutputType;
...
public:
  MonoUnpackedDecoder(QSize size_)
    : size(size_)
    , M(size_.height(), size_.width(), cvMatType)
  {
  }
  ...
  const cv::Mat getCvImage() { return M; }
};
}
