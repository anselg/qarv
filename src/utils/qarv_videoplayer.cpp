...
QArvVideoPlayer::QArvVideoPlayer(QString filename, QWidget* parent,
                                 Qt::WindowFlags f)
  : QWidget(parent, f)
{
  setupUi(this);
...
bool
QArvVideoPlayer::open(QString filename)
{
...
void
QArvVideoPlayer::on_playButton_toggled(bool checked)
{
  if (checked) {
...
void
QArvVideoPlayer::on_openButton_clicked(bool checked)
{
...
void
QArvVideoPlayer::readNextFrame(bool seeking)
{
...
void
QArvVideoPlayer::on_slider_valueChanged(int value)
{
...
void
QArvVideoPlayer::on_transcodeBox_toggled(bool checked)
{
...
void
QArvVideoPlayer::on_leftMarkButton_clicked(bool checked)
{
  leftFrame = checked ? slider->value() : 0;
}

void
QArvVideoPlayer::on_rightMarkButton_clicked(bool checked)
{
  rightFrame = checked ? slider->value() : slider->maximum();
}

void
QArvVideoPlayer::on_transcodeButton_toggled(bool checked)
{
  if (checked) {
    ...
    auto fmt = qvariant_cast<OutputFormat*>(
      codecBox->itemData(codecBox->currentIndex()));
    if (fmt) {
      ...
      recorder.reset(
        makeGstRecorder(QStringList(), gstLine->text(), decoder.data(), fname,
                        recording->frameSize(), fpsSpinbox->value(), false));
    } else {
      transcodeButton->setChecked(false);
      return;
    }
