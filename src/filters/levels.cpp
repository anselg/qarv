...
LevelsFilter::LevelsFilter(ImageFilterPlugin* plugin) : ImageFilter(plugin) {
	black.store(0);
	white.store(1);
	gamma.store(1);
}
...
LevelsSettingsWidget::LevelsSettingsWidget(ImageFilter* filter_, QWidget* parent, Qt::WindowFlags f) 
