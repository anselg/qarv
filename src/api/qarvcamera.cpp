		...
		case Qt::EditRole:
			...
			if (ARV_IS_GC_ENUMERATION(node)) {
				...
				for (; entry != NULL; entry = entry->next) {
					...
					const char* name =
						arv_gc_feature_node_get_display_name(ARV_GC_FEATURE_NODE(entry->data),NULL);
					if (name == NULL)
...
bool QArvCamera::setData(const QModelIndex& index, const QVariant& value, int role) {
	...
	if (value.canConvert<QArvRegister>()) {
		...
		if (e.isAvailable.at(e.currentValue)) {
			arv_gc_enumeration_set_string_value(ARV_GC_ENUMERATION(node), 
			                                    e.values.at(e.currentValue).toLatin1().data(),
			                                    NULL);
		} else return false;
		...
		arv_gc_string_set_value(ARV_GC_STRING(node), s.value.toLatin1().data(), NULL);
...
QVariant QArvCamera::headerData(int section, Qt::Orientation orientation, int role) const {
	switch (section) {
	case 0:
