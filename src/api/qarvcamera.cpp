...
void
QArvCamera::setBinning(QSize bin)
{
...
QList<QString>
QArvCamera::getPixelFormatNames()
{
  unsigned int numformats;
  const char** formats =
...
QArvCamera::getPixelFormatIds()
{
  unsigned int numformats;
  gint64* formats = arv_camera_get_available_pixel_formats(camera, &numformats);
...
double
QArvCamera::getExposure()
{
  return arv_camera_get_exposure_time(camera);
}

void
QArvCamera::setExposure(double exposure)
{
...
bool
QArvCamera::hasAutoExposure()
{
  return arv_camera_is_exposure_auto_available(camera);
}

void
QArvCamera::setAutoExposure(bool enable)
{
  if (enable)
...
void
QArvCamera::setGain(double gain)
{
...
bool
QArvCamera::hasAutoGain()
{
  return arv_camera_is_gain_auto_available(camera);
}

void
QArvCamera::setAutoGain(bool enable)
{
  if (enable)
...
inline void
QArvStreamCallback(void* vcam, int type, ArvBuffer* bfr)
{
...
static void
QArvStreamCallbackWrap(void* vcam, ArvStreamCallbackType type, ArvBuffer* bfr)
{
 ...
 */
void
QArvCamera::startAcquisition(bool zeroCopy, bool dropInvalidFrames)
{
  nocopy = zeroCopy;
 ...
 * should be used when using the zeroCopy facility of startAcquisition(), as
 * this
 ...
 */
void
QArvCamera::setFrameQueueSize(uint size)
{
  frameQueueSize = size;
}
...
static QHostAddress
GSocketAddress_to_QHostAddress(GSocketAddress* gaddr)
{
  sockaddr addr;
...
int
QArvCamera::getEstimatedBW()
{
  return arv_device_get_integer_feature_value(device, "GevSCDCT");
}

QTextStream&
operator<<(QTextStream& out, QArvCamera* camera)
{
  auto id = camera->getId();
  out << id.vendor << endl << id.model << endl << id.id << endl;
  QArvCamera::QArvFeatureTree::recursiveSerialization(out, camera,
                                                      camera->featuretree);
  return out;
}
 ...
 */
QTextStream&
operator>>(QTextStream& in, QArvCamera* camera)
{
  auto ID = camera->getId();
  ...
  while (!in.atEnd()) {
    ...
    ArvGcFeatureNode* node =
      ARV_GC_FEATURE_NODE(arv_gc_get_node(camera->genicam, name.toLatin1()));
...
QModelIndex
QArvCamera::index(int row, int column, const QModelIndex& parent) const
{
  if (column > 1)
    return QModelIndex();
  ...
  if (ARV_IS_GC_CATEGORY(node) && column > 0)
    return QModelIndex();
  return createIndex(row, column, child);
}

QModelIndex
QArvCamera::parent(const QModelIndex& index) const
{
  QArvCamera::QArvFeatureTree* treenode;
  if (!index.isValid())
    treenode = featuretree;
...
int
QArvCamera::columnCount(const QModelIndex& parent) const
{
  return 2;
}

int
QArvCamera::rowCount(const QModelIndex& parent) const
{
  QArvCamera::QArvFeatureTree* treenode;
  if (!parent.isValid())
    treenode = featuretree;
...
QVariant
QArvCamera::data(const QModelIndex& index, int role) const
{
  QArvCamera::QArvFeatureTree* treenode;
  if (!index.isValid())
    treenode = featuretree;
  ...
  if (!ARV_IS_GC_FEATURE_NODE(node)) {
    logMessage() << "data:"
                 << "Node" << treenode->feature() << "is not valid!";
    return QVariant();
  }
  ...
  if (index.column() == 0) {
    switch (role) {
      ...
      case Qt::WhatsThisRole:
        ...
        string2 = arv_gc_feature_node_get_name(ARV_GC_FEATURE_NODE(node));
    ...
    switch (role) {
      ...
      case Qt::EditRole:
        if (ARV_IS_GC_REGISTER_NODE(node) &&
          ...
          arv_gc_register_get(ARV_GC_REGISTER(node), r.value.data(), r.length,
                              NULL);
        ...
        if (ARV_IS_GC_ENUMERATION(node)) {
          ...
          for (; entry != NULL; entry = entry->next) {
            bool isAvailable = arv_gc_feature_node_is_available(
              ARV_GC_FEATURE_NODE(entry->data), NULL);
            bool isImplemented = arv_gc_feature_node_is_implemented(
              ARV_GC_FEATURE_NODE(entry->data), NULL);
            e.values << arv_gc_feature_node_get_name(
              ARV_GC_FEATURE_NODE(entry->data));
            const char* name = arv_gc_feature_node_get_display_name(
              ARV_GC_FEATURE_NODE(entry->data), NULL);
            if (name == NULL)
...
bool
QArvCamera::setData(const QModelIndex& index, const QVariant& value, int role)
{
  ...
  if (value.canConvert<QArvRegister>()) {
    ...
    if (e.isAvailable.at(e.currentValue)) {
      arv_gc_enumeration_set_string_value(
        ARV_GC_ENUMERATION(node), e.values.at(e.currentValue).toLatin1().data(),
        NULL);
    } else
      return false;
...
Qt::ItemFlags
QArvCamera::flags(const QModelIndex& index) const
{
  ...
  if (index.column() != 1 && !ARV_IS_GC_CATEGORY(node)) {
    ...
    int enabled = arv_gc_feature_node_is_available(node, NULL) &&
...
QVariant
QArvCamera::headerData(int section, Qt::Orientation orientation, int role) const
{
  switch (section) {
    case 0:
...
QList<QString>
QArvCamera::features(const QString& category) const
{
  QList<QString> list;
  for (int i = 0; i < rowCount(); i++) {
    auto idx = index(i, 0);
...
QModelIndex
QArvCamera::featureIndex(const QString& feature) const
{
  for (int i = 0; i < rowCount(); i++) {
    auto idx = index(i, 0);
    for (int j = 0; j < rowCount(idx); j++) {
