...
QArvGui::init(QApplication* a)
{
  ...
  // Install a global event filter that makes sure that long tooltips can be
  // word-wrapped
  ...
  : QWidget(parent)
{
  ...
  connect(ext->mw, SIGNAL(recordingStarted(bool)),
...
void
QArvGui::signalForwarding(bool enable)
{
  if (enable) {
    connect(ext->mw->workthread, SIGNAL(frameDelivered(QByteArray, ArvBuffer*)),
            this, SIGNAL(frameReady(QByteArray, ArvBuffer*)));
    connect(ext->mw->workthread, SIGNAL(frameCooked(cv::Mat)), this,
            SIGNAL(frameReady(cv::Mat)));
  } else {
    disconnect(ext->mw->workthread,
               SIGNAL(frameDelivered(QByteArray, ArvBuffer*)), this,
               SIGNAL(frameReady(QByteArray, ArvBuffer*)));
    disconnect(ext->mw->workthread, SIGNAL(frameCooked(cv::Mat)), this,
               SIGNAL(frameReady(cv::Mat)));
  }
 ...
 */
void
QArvGui::forceRecording()
{
  if (!ext->mw->standalone) {
    if (!ext->mw->recordAction->isChecked())
      ext->mw->recordAction->setChecked(true);
...
void
QArvGui::closeEvent(QCloseEvent* event)
{
