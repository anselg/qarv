...
bool
QArvRecordedVideo::status()
{
  return isOK && (videofile.error() == QFile::NoError);
}

QFile::FileError
QArvRecordedVideo::error()
{
  return videofile.error();
}

QString
QArvRecordedVideo::errorString()
{
  return videofile.errorString();
}

bool
QArvRecordedVideo::atEnd()
{
  return videofile.atEnd();
}

bool
QArvRecordedVideo::isSeekable()
{
  return uncompressed;
}

int
QArvRecordedVideo::framerate()
{
  return fps;
}

QSize
QArvRecordedVideo::frameSize()
{
  return fsize;
}

uint
QArvRecordedVideo::frameBytes()
{
  return frameBytes_;
}

QArvDecoder*
QArvRecordedVideo::makeDecoder()
{
  if (!isOK)
    return NULL;
  if (arvPixfmt != 0) {
...
bool
QArvRecordedVideo::seek(uint frame)
{
  return videofile.seek(frame * frameBytes_);
}

QByteArray
QArvRecordedVideo::read()
{
  return videofile.read(frameBytes_);
}

uint
QArvRecordedVideo::numberOfFrames()
{
