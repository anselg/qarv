...
void
CV2QImage_RGB24Template(const cv::Mat& image, QImage& img)
{
...
void
QArvDecoder::CV2QImage_RGB24(const cv::Mat& image, QImage& out)
{
  switch (image.type()) {
    case CV_16UC1:
...
QImage
QArvDecoder::CV2QImage_RGB24(const cv::Mat& image)
{
  QImage img;
...
template <bool grayscale, bool depth8>
void
CV2QImageTemplate(const cv::Mat& image_, QImage& image)
{
  typedef typename std::conditional<depth8, uint8_t, uint16_t>::type InputType;
...
void
QArvDecoder::CV2QImage(const cv::Mat& image, QImage& out)
{
  switch (image.type()) {
    case CV_16UC1:
...
QImage
QArvDecoder::CV2QImage(const cv::Mat& image)
{
  QImage img;
  CV2QImage(image, img);
  return img;
 ...
 */
QArvDecoder*
QArvDecoder::makeDecoder(QByteArray specification)
{
 ...
 */
QArvDecoder*
QArvDecoder::makeDecoder(ArvPixelFormat format, QSize size, bool fast)
{
  foreach (auto fmt, pluginFormats) {
    if (fmt->pixelFormat() == format)
      return fmt->makeDecoder(size);
  }
