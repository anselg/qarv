...
QArvDecoder* QArvDecoder::makeDecoder(QByteArray specification) {
	static QStringList types = { "Aravis", "SwScale", };
 ...
 */
QArvDecoder* QArvDecoder::makeDecoder(ArvPixelFormat format, QSize size, 
                                      bool fast) {
	foreach (auto fmt, pluginFormats) {
 ...
 * Returns NULL if the format is not supported.
 */
QArvDecoder* QArvDecoder::makeSwScaleDecoder(PixelFormat fmt, QSize size, 
                                             int swsFlags) {
	if (swsFlags)
